...
SELECT pid AS 產品名稱,
	   name AS 產品名稱,
       price AS 銷售價格
FROM products WHERE price BETWEEN 20 AND 40;
...
SELECT * FROM products WHERE category_id IN(1,3,5,7) ORDER BY category_id ASC; 
...
SELECT * FROM products WHERE name LIKE '%米%' AND name NOT LIKE '玉米%' AND name NOT LIKE '蝦米' ;

SELECT * FROM products WHERE name LIKE '%米%' AND category_id=1;

#聚合函數, 群組
SELECT category_id AS 產品類別, COUNT(category_id) AS 各項類別總數 FROM products GROUP BY category_id;

#條件子句
SELECT category_id AS 產品類別, COUNT(category_id) AS 各項類別總數 FROM products GROUP BY category_id HAVING 各項類別總數 >=10 ; 
SELECT category_id AS 產品類別, COUNT(category_id) AS 各項類別總數 FROM products GROUP BY category_id HAVING COUNT(category_id) >=10 ; 

SELECT unit AS 以箱為單位,
	   COUNT(unit) AS 總數
FROM products GROUP BY unit HAVING unit LIKE '每箱%';
...
SELECT name AS 產品名稱,
       ...
       SUM(qty) AS 總數量,
       COUNT(id) AS 總筆數
       FROM products,order_detail WHERE products.pid = order_detail.pid 
       GROUP BY name;
 ...
 SELECT name AS 產品名稱,
       ...
       SUM(qty) AS 總數量,
       COUNT(id) AS 總筆數
       FROM products LEFT JOIN order_detail ON products.pid = order_detail.pid 
       GROUP BY name;
